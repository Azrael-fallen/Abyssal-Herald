...
class AbyssalHeraldBot extends Client {
  constructor() {
    super({
      intents: [
  ...
  async start() {
    try {
      startupLog('Starting Abyssal Herald...');
  ...
  startWebServer() {
    ...
    app.get('/', (req, res) => {
      res.status(200).json({ 
        message: 'Abyssal Herald System Online',
        version: '2.0.0',
        timestamp: new Date().toISOString()
      });
    });
...
try {
  const bot = new AbyssalHeraldBot();
...
export default AbyssalHeraldBot;
